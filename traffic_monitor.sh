...
run_monitor() {
    ...
    ipset create whitelist_v6 hash:ip family inet6 timeout 0 2>/dev/null || true
    ipset create banlist hash:ip timeout 86400 2>/dev/null || true
    ipset create banlist_v6 hash:ip family inet6 timeout 86400 2>/dev/null || true
...
install_monitor() {
    ...
    ipset create whitelist_v6 hash:ip family inet6 2>/dev/null || true
    ipset create banlist hash:ip timeout 86400 2>/dev/null || true
    ipset create banlist_v6 hash:ip family inet6 timeout 86400 2>/dev/null || true
...
add_whitelist_batch() {
    echo -e "${CYAN}输入IP(支持IPv4/IPv6, 空格分隔, 0结束):${NC}"
...
manage_whitelist() {
    while true; do
        ...
        ipset list whitelist_v6 &>/dev/null || ipset create whitelist_v6 hash:ip family inet6 2>/dev/null
...
manage_blacklist() {
    while true; do
        ...
        ipset list banlist_v6 &>/dev/null || ipset create banlist_v6 hash:ip family inet6 timeout 86400 2>/dev/null
        echo -e "${CYAN}1.${NC} 添加IP\n${CYAN}2.${NC} 查看列表\n${CYAN}3.${NC} 删除IP\n${CYAN}0.${NC} 返回\n"
        read -p "选择: " c
        case $c in
            1)

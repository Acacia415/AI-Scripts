...
shell_version="2.4.0" # Version updated: fix wss/ws path & cert persistence
...
function eachconf_retrieve() {
  ...
  # 从 rawconf 提取额外选项 (格式: type/port#ip#dport 或 type/port#ip#dport#options)
  # options 格式: key1=val1,key2=val2
  local remaining="${d_port#*#}"
  if [[ "$d_port" == *"#"* ]]; then
    d_port="${d_port%%#*}"
    extra_opts="$remaining"
  else
    extra_opts=""
  fi
  # 解析 extra_opts
...
function encrypt() {
  ...
  if [[ "$numencrypt" == "2" ]] || [[ "$numencrypt" == "3" ]]; then
    read -e -p "请输入ws路径 (默认 /ws): " ws_path
    ws_path=${ws_path:-/ws}
  fi
}
...
function decrypt() {
  ...
  if [[ "$numdecrypt" == "2" ]] || [[ "$numdecrypt" == "3" ]]; then
    read -e -p "请输入ws路径 (默认 /ws): " ws_path
    ws_path=${ws_path:-/ws}
  fi
}
...
function writerawconf() {
  ...
  # 持久化额外选项到 rawconf
  if [[ ${is_cert} == [Yy] ]]; then
    extra_opts="cert=${is_cert}"
  fi
  if [[ -n "$ws_path" && "$ws_path" != "/ws" ]]; then
    if [[ -n "$extra_opts" ]]; then
      extra_opts="${extra_opts},path=${ws_path}"
    else
      extra_opts="path=${ws_path}"
    fi
  fi
  if [[ -n "$extra_opts" ]]; then
    echo "${flag_a}/${flag_b}#${flag_c}#${flag_d}#${extra_opts}" >> "$raw_conf_path"
  else

...
CYAN='\033[36m'
NC='\033[0m'
# 初始化标记文件路径（解决sudo权限问题）
USER_HOME=$(eval echo ~${SUDO_USER:-$USER})
INSTALL_MARKER="$USER_HOME/.iris_toolbox_installed"
# ======================= 快捷键设置 =======================
setup_alias() {
    # 定义别名命令
    ...
    [[ -f $USER_HOME/.zshrc ]] && config_files+=("$USER_HOME/.zshrc")
    [[ -f $USER_HOME/.bash_profile ]] && config_files+=("$USER_HOME/.bash_profile")
    # 如果没有找到配置文件则跳过
...
check_first_run() {
    if [[ ! -f "$INSTALL_MARKER" ]]; then
        echo -e "${CYAN}[系统初始化] 检测到首次运行，正在配置快捷方式...${NC}"
        setup_alias
    fi
}
# ======================= 主逻辑 =======================
...
uninstall_service() {
    ...
    # 新增清理步骤
    echo -e "\n${YELLOW}[附加清理] 移除快捷配置...${NC}"
    # 删除别名配置
    sed -i '/# IRIS工具箱快捷命令/d' \
        "$USER_HOME/.bashrc" \
        "$USER_HOME/.zshrc" \
        "$USER_HOME/.bash_profile" 2>/dev/null
    # 删除标记文件
    rm -f "$INSTALL_MARKER"
    # 取消当前会话的别名

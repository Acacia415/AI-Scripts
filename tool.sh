...
main_menu() {
  while true; do
    ...
    printf "%-40s %s\n" "3. 智能流量监控" "19. TCP性能优化"
    printf "%-40s %s\n" "4. 安装 Snell 协议服务" "20. 命令行美化"
    printf "%-40s %s\n" "5. 安装 Hysteria2 协议服务" "21. DNS解锁服务"
    printf "%-40s %s\n" "6. 安装 SS-Rust 协议服务" "22. 安装Sub-Store"
    printf "%-40s %s\n" "7. 安装 ShadowTLS" "23. 搭建TG图床"
    printf "%-40s %s\n" "8. 一键IPTables转发" "24. TCP性能优化 (BBR+fq)"
    printf "%-40s %s\n" "9. 一键GOST转发" "25. 恢复TCP原始配置"
    printf "%-40s %s\n" "10. 安装 3X-UI 管理面板" "26. 安装Fail2Ban"
    printf "%-40s %s\n" "11. 流媒体解锁检测" "27. 安装 acme.sh"
    printf "%-40s %s\n" "12. Speedtest网络测速" "28. 安装 Gost v3"
    printf "%-40s %s\n" "13. BestTrace回程测试" "29. 修改主机名"
    printf "%-40s %s\n" "14. 开放所有端口" "30. 重装系统"
    printf "%-40s %s\n" "15. 时间同步" "31. 部署Hexo博客"
    printf "%-40s %s\n" "16. Caddy反代管理" "32. 安装Hexo_butterfly主题"
    echo -e "${YELLOW}==========================================================================${NC}"
    echo "0. 退出脚本"
    echo -e "${YELLOW}-------------------------------------------------------------------------${NC}"

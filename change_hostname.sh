...
update_hosts_file() {
    ...
    temp=$(mktemp /etc/.hosts.ai-scripts.XXXXXX)
    awk -v host="$value" '
        BEGIN { found_loopback=0; found_host=0 }
        /^[[:space:]]*127\.0\.0\.1([[:space:]]|$)/ {
            if (!found_loopback) { print "127.0.0.1\tlocalhost"; found_loopback=1 }
            next
        }
...
change_hostname_safely() {
    ...
    if ! set_hostname_value "$new_hostname" || ! update_hosts_file "$new_hostname"; then
